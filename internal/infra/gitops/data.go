...
// ReadFile reads raw bytes of relPath from the remote branch.
func (c *Committer) ReadFile(ctx context.Context, relPath string) ([]byte, error) {
...
// ReadDocument reads relPath from git and extracts its data map and annotations if it's a YAML document.
func (c *Committer) ReadDocument(ctx context.Context, relPath string) (data, annotations map[string]string, raw []byte, err error) {
	raw, err = c.ReadFile(ctx, relPath)
	if err != nil {
	...
	return data, annotations, raw, nil
}

// PatchDocument loads a YAML document at relPath, passes its data and annotations maps to mutate,
// and if changed commits the updated YAML.
func (c *Committer) PatchDocument(ctx context.Context, relPath, commitMsg string,
	mutate func(data, annotations map[string]string) (bool, error)) (bool, error) {
	return c.mutateDoc(ctx, relPath, commitMsg, func(root *yaml.Node) (bool, error) {
		dataNode := mapValue(root, "data")
		if dataNode == nil {
			return false, fmt.Errorf("no data map in %s", relPath)
		...
		changed, err := mutate(data, annotations)
		if err != nil || !changed {
			return false, err
		}
		...
		if metaNode != nil && len(annotations) > 0 {
			...
			if annNode == nil {
				kn := &yaml.Node{Kind: yaml.ScalarNode, Tag: "!!str", Value: "annotations"}
				annNode = &yaml.Node{Kind: yaml.MappingNode, Tag: "!!map"}
				metaNode.Content = append(metaNode.Content, kn, annNode)
			}
			annChanged = replaceData(annNode, annotations)
		}
		return dataChanged || annChanged, nil
	})
}
