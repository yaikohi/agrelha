...
func (h *Handler) ModUpdateState(ctx context.Context, d *pages.InstanceDetailUI, inst domain.Instance) {
	...
	if inst.State == domain.StateRunning {
		if st, ok := h.InstanceStats(ctx, []domain.Instance{inst})[inst.Number]; ok {
			d.Players, d.PlayersKnown = st.Players, st.PlayersKnown
		}
	}
}

// MCModUpdatesCheck re-checks one world against Modrinth on demand,
// rather than waiting for the background refresh.
func (h *Handler) MCModUpdatesCheck(c *fiber.Ctx) error {
	...
	if n := len(rep.Missing); n > 0 {
		msg = fmt.Sprintf("%d installed mod(s) are gone from Modrinth — this world will fail to boot.", n)
		kind = "err"
	}
	return h.pushUpdatePanel(c, *inst, msg, kind)
}

// MCModUpdatesApply repins the selected mods to their latest versions.
func (h *Handler) MCModUpdatesApply(c *fiber.Ctx) error {
	inst, err := h.updatableInstance(c)
	if err != nil {
		return shared.SSEToast(c, "err", err.Error(), nil)
	}
...
// MCModUpdatesUndo restores the mod list from before the last update.
func (h *Handler) MCModUpdatesUndo(c *fiber.Ctx) error {
	inst, err := h.updatableInstance(c)
	if err != nil {
		return shared.SSEToast(c, "err", err.Error(), nil)
	}
...
func (h *Handler) updatableInstance(c *fiber.Ctx) (*domain.Instance, error) {
	if h.cfg.MCInstances == nil {
		return nil, fmt.Errorf("Minecraft instance manager unconfigured")
	}
	...
	if err != nil || inst == nil {
		return nil, fmt.Errorf("Minecraft instance not found")
	}
	if inst.IsVanilla() {
		return nil, inst.VanillaImmutableErr()
	}
	if inst.PackDefined() {
		return nil, fmt.Errorf("modpack-defined instance mods are managed by the modpack")
	}
	return inst, nil
}
